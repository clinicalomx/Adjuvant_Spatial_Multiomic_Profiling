...
def _gcross_single_sample_single_pair(
    ...
    target_ix = np.where(adata_subset.obs[cell_type_key] == target_cell_type)[0]
    # Check if 
    if len(query_ix) < minimum_cell_count or len(target_ix) < minimum_cell_count:
    ...
    if n_processes == 1:
        for sample in samples:
            ...
            for query_cell_type, target_cell_type in pairwise_combinations:
                ...
                g_cross_results[sample][(query_cell_type, target_cell_type)] = xs, g_cross
    else:
        ...
        g_cross_results = {
            ...
            for args_tuple, result in zip(args, results)
        }
...
def gcross_subset_border_correction(
    ...
    for r in bins:
        ...
        # f(w, t) x f(t, b)
        numerator = neighbor_distances_within_r & \
            r_within_edge_dists
        ...
        if r_within_edge_dists.sum() <= 0:
            ...
            g_cross.append(
                numerator.sum() / r_within_edge_dists.sum())
...
def gcross_subset_hanisch_unbiased_correction(
    ...
    areas = get_hanisch_area(
        spatial_coordinates[query_indices, :],        
        distances_to_edge
    )
    ...
    for r in bins:
        ...
        # f(w, t) x f(w, b)
        numerator = neighbor_distances_within_r & \
            neighbor_distances_within_edge_dists
...
def gcross_subset_kaplan_meier_correction(
    ...
    g_cross = _compute_kaplan_meier_estimate(distances, distances_to_edge, bins)
...
def _compute_kaplan_meier_estimate(
    ...
    binned_uncensored_distances, _ = np.histogram(min_edge_neighbour_distance[is_uncensored], bins=bins)
    ...
    number_greater_uncensored = binned_distances[::-1].cumsum()[::-1] + number_distances_above_max
    ...
    with np.errstate(invalid='ignore'):
        km_factors = np.where(number_greater_uncensored > 0,
...
if __name__ == "__main__":
    adata = ad.read_h5ad("../../../ytma_nsclc_2025/2-output_nsclc_ytma_features_preandpost.h5ad")
    start = time.time()
    results = gcross(
        adata=adata,
        ...
        spatial_key="spatial",
        # query_cell_types="macrophagecells",
        # target_cell_types="tumorcells",
        n_processes=1,
        correction="km",
    )
    end = time.time() - start
    ...
    # print(df)
    # xs, gcross_vals = df.loc[34716][("macrophagecells", "tumorcells")]
    # plt.plot(xs, gcross_vals)
    # plt.show()

# Temps
# adata_subset = adata[adata.obs["patientid"] == 31590]
# spatial_coordinates = adata_subset.obsm["spatial"]
# query_ix = np.where(adata_subset.obs["celltypes"] == "macrophagecells")[0]
# target_ix = np.where(adata_subset.obs["celltypes"] == "tumorcells")[0]
# nquery_indices = adata_subset.obs.loc[query_ix].reset_index().index
# target_indices = adata_subset.obs.loc[target_ix].reset_index().index
